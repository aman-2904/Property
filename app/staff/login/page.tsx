...
import { Lock, Mail, Loader2, AlertCircle, ShieldAlert, ArrowLeft, CheckCircle, Key, Eye, EyeOff, Contact } from "lucide-react";
...
export default function StaffLoginPage() {
  ...
  const onSubmit = (data: LoginFormValues) => {
    ...
    startTransition(async () => {
      // Call login server action with loginType = 'staff'
      const res = await login(data, 'staff');
      if (res && res.error) {
        setError(res.error);
      }
    });
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    if (!resetEmail.trim()) {
      setResetError("Please enter your staff email address.");
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        setResetSuccess("Verification OTP sent to your email! Please check your inbox.");
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent">
              {forgotPasswordStep ? "Reset Password" : "Staff CRM Portal"}
            </h1>
            <p className="text-sm text-muted-foreground mt-2">
              {forgotPasswordStep === "request" && "Enter your email to receive reset OTP"}
              {forgotPasswordStep === "verify" && "Enter the 6-digit OTP and new password"}
              {!forgotPasswordStep && "Staff Portal. Please sign in to manage leads."}
            </p>
          </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-muted/20 border border-border/50 rounded-xl text-sm outline-none focus:border-emerald-500/50 transition-all"
                    required
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-muted/20 border border-border/50 rounded-xl text-sm outline-none focus:border-emerald-500/50 transition-all font-mono tracking-[0.3em] text-center text-lg font-semibold"
                    required
                  />
                </div>
              </div>
          ...
          <div className="mt-8 pt-6 border-t border-border/40 flex flex-col gap-3 items-center text-xs text-muted-foreground text-center">
            <div>
              Don&apos;t have a Staff account?{" "}
              <Link href="/staff/register" className="text-emerald-400 hover:underline font-semibold">
                Register Staff
              </Link>
            </div>
            <Link
              href="/login"
              className="flex items-center gap-1.5 hover:text-emerald-400 font-medium transition-colors mt-1"
            >
